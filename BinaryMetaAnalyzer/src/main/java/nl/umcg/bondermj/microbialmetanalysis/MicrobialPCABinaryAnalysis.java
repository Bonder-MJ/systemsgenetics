 ...
 * @author Harm-Jan
 */
public class MicrobialPCABinaryAnalysis {
    ...
    public static void main(String[] args) {
        ...
        MicrobialPCABinaryAnalysis meta = new MicrobialPCABinaryAnalysis(settingsFile, textToReplace, replaceTextWith);
        System.exit(0);
    ...
    public MicrobialPCABinaryAnalysis(String settingsFile, String textToReplace, String replaceTextWith) {
        // initialize settings
        ...
        try {
            ...
            Logger.getLogger(MicrobialPCABinaryAnalysis.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void run() throws IOException {
        ...
        for (int permutation = 0; permutation < settings.getNrPermutations() + 1; permutation++) {
            ...
            datasets = new BinaryMicrobiomMetaAnalysisDataset[settings.getDatasetlocations().size()];

            System.out.println("Loading datasets");
            for (int d = 0; d < datasets.length; d++) {
                datasets[d] = new BinaryMicrobiomMetaAnalysisDataset(settings.getDatasetlocations().get(d), settings.getDatasetPrefix().get(d), permutation, settings.getDatasetannotations().get(d), probeAnnotation);
            }
...
//                                MetaQTL4MetaTrait[] datasetCisProbes = datasets[d].getProbes(datasetSNPId);
//
...
//                }
            ...
            for (BinaryMicrobiomMetaAnalysisDataset dataset : datasets) {
                dataset.close();
            }
